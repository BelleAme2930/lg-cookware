...
const SaleInvoice = ({ sale }) => {
    ...
    return (
        <div className="min-h-screen bg-gray-50 py-6">
            <Head title={`Invoice: INV-P-${sale.id}`}/>
            ...
            <ShadowBox className="max-w-[1920px] w-[90%] mx-auto">
                <div className="p-6 flex justify-between items-center border-b">
                    ...
                    <div className="text-right">
                        <h2 className="text-xl font-semibold">Invoice: {`INV-P-${sale.id}`}</h2>
                    </div>
                </div>
                ...
                <div className="bg-gray-50 p-6">
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        <div>
                            <p className="mb-2"><span className="font-medium">Invoice ID:</span> {`INV-P-${sale.id}`}</p>
                            <p><span className="font-medium">Customer:</span> {sale.customer?.name}</p>
                        </div>
                        <div>
                            <p className="mb-2"><span className="font-medium">Sale Date:</span> {sale.sale_date_display}</p>
                            <p>
                                <span className="font-medium">Payment Method:</span> <span className='capitalize'>{sale.payments.map(p => p.method).join(', ')}</span>
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="flex justify-end">
                        <div className="w-64">
                            <div className="flex justify-between mb-2">
                                ...
                                <span>{sale.total_amount_formatted} Rs.</span>
                            </div>
                            <div className="flex justify-between mb-2">
                                <span className="font-medium">Remaining Balance:</span>
                                <span>{sale.remaining_balance_formatted || "0"} Rs.</span>
                            </div>
                            <div className="flex justify-between font-bold text-lg border-t pt-2 mt-2">
                                <span>Total Price:</span>
                                <span className="text-green-600">{sale.total_amount_formatted} Rs.</span>
                            </div>
                        </div>
                    </div>
                </div>
